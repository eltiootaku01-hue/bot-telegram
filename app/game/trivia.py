...
from app.db.models import TriviaAttempt, TriviaRound
...
QUESTIONS: tuple[TriviaQuestion, ...] = (
    TriviaQuestion(
        "¿En qué anime aparece el personaje Levi Ackerman?",
        ("Naruto", "Shingeki no Kyojin", "Bleach", "Black Clover"),
        1,
        "Levi es capitán del Cuerpo de Exploración en Shingeki no Kyojin.",
        20,
    ),
    TriviaQuestion(
        "¿Cómo se llama la hermana menor de Tanjiro Kamado?",
        ("Nezuko", "Shinobu", "Kanao", "Mitsuri"),
        0,
        "Nezuko es la hermana menor de Tanjiro.",
        20,
    ),
    TriviaQuestion(
        "¿Qué objeto utiliza Usagi Tsukino para transformarse en Sailor Moon?",
        ("Un anillo", "Un broche", "Una espada", "Un reloj"),
        1,
        "El broche es uno de los objetos emblemáticos de sus transformaciones.",
        15,
    ),
    TriviaQuestion(
        "¿Quién es el protagonista de One Punch Man?",
        ("Genos", "Garou", "Saitama", "King"),
        2,
        "Saitama derrota a sus enemigos de un solo golpe.",
        15,
    ),
    TriviaQuestion(
        "¿Qué criatura acompaña normalmente a Pikachu?",
        ("Ash", "Luffy", "Gon", "Ichigo"),
        0,
        "Ash Ketchum es el compañero más conocido de Pikachu.",
        10,
    ),
)
...
class TriviaService:
    """Small deterministic anime quiz engine; no LLM/API call is required."""
    ...
    ) -> tuple[TriviaRound, TriviaQuestion] | None:
    ...
    async def answer(
        ...
        """Return (result, new_balance). Only one user can win a round."""
        ...
        claimed = await session.execute(
            update(TriviaRound)
            .where(TriviaRound.id == round_id, TriviaRound.status == "active")
