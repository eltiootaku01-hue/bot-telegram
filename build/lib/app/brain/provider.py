...
class BrainClient:
    ...
    async def generate(self, request: LLMRequest) -> str:
        ...
        errors: list[str] = []
        for provider in DynamicLLMPool.configs():
            try:
                result = await asyncio.to_thread(
                    DynamicLLMPool.generate,
                    provider,
                    request,
                    self._post_json,
                    self._messages,
                ...
                logger.warning("Dynamic LLM provider %s failed: %s", provider.get("alias", provider.get("provider")), exc)
                errors.append(f"{provider.get('alias', provider.get('provider'))}: {exc}")
        ...
        logger.warning("LLM pool exhausted; using SQLite offline response: %s", "; ".join(errors))
        return self._clean(DynamicLLMPool.offline(request, self.settings.database_url))
