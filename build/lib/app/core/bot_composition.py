...
from src.bot.modules import SunnaNativeTriviaModule
...
def build_bot_modules(
    ...
    shared = (
        ...
        _spec(
            "social-runtime",
            lambda: SocialRuntimeModule(database, identity, settings=settings)
        ),
    )
    identity_specific = (
        ...
        _spec("native-trivia", lambda: SunnaNativeTriviaModule(database), BotIdentity.SUNNA),
    ...
    return BotComposition((*shared, *identity_specific)).build(identity)
