...
async def build_dispatcher(
...
) -> tuple[Dispatcher, dict[BotIdentity, Bot], Database, VaultClient]:
    ...
    conversation = ConversationEngine(dialogues)
    ...
    return dp, bots, database, vault


async def run() -> None:
    config = MultiBotConfig.from_env()
    dp, bots, database, vault = await build_dispatcher(config)
    logger.info(
