...
class ProviderManager:
    ...
    def __init__(self, providers: tuple[BaseProvider, ...], provider_configs: dict | None = None) -> None:
        ...
        for provider in providers:
            ...
            self._health[(provider.provider_id, account_id)] = ProviderHealthRecord(
    ...
    def execute(
        ...
        *,
        ...
        fallback_accounts: tuple[str, ...] = (),
    ) -> ProviderOutcome:
        if not (decision.requires_llm or decision.requires_search):
            return ProviderOutcome(
                ProviderResponse(
                    provider="none", model=request.model, status=ProviderStatus.SKIPPED,
                    output_text=None, usage=ProviderUsage(), latency_ms=0,
        ...
        if last_error is None:
            return ProviderOutcome(
                self._error(request, "No provider account available", FailureClass.CONFIGURATION),
                tuple(attempts),
            )
        return ProviderOutcome(
            self._error(request, type(last_error).__name__, classify_provider_error(last_error)),
            tuple(attempts),
        )
    ...
    def _make_request(self, request: ProviderRequest, provider_id: str, account_id: str | None, provider: BaseProvider | None) -> ProviderRequest:
        ...
        return ProviderRequest(
            ...
            max_output_tokens=max_tokens or request.max_output_tokens,
            timeout_seconds=timeout or request.timeout_seconds,
            request_id=request.request_id,
    ...
    def _call(self, request: ProviderRequest, attempts: list[str], provider: BaseProvider) -> ProviderResponse:
        ...
        if len(attempts) > 1:
            return ProviderResponse(
                provider=response.provider, model=response.model, status=response.status,
                output_text=response.output_text, usage=response.usage, latency_ms=response.latency_ms,
                request_id=response.request_id, account_id=response.account_id,
                fallback_used=True, fallback_from=attempts[0],
            )
        return response
    ...
    def _error(request: ProviderRequest, error_type: str, failure_class: FailureClass) -> ProviderResponse:
        return ProviderResponse(
            provider=request.provider, model=request.model, status=ProviderStatus.ERROR,
            output_text=None, usage=ProviderUsage(), latency_ms=0,
            request_id=request.request_id, error_type=error_type,
