...
The SQLite ``memories`` table remains authoritative. This index stores only
searchable projections and can be discarded/rebuilt at any time.
"""
...
class MemorySearchIndex:
    """Rebuildable FTS5 projection; gracefully unavailable when SQLite lacks FTS5."""
    ...
    def _probe(connection: sqlite3.Connection) -> bool:
        try:
            connection.execute("CREATE VIRTUAL TABLE IF NOT EXISTS temp.bot_ia_fts_probe USING fts5(value)")
    ...
    def _ensure_schema(self, connection: sqlite3.Connection) -> None:
        connection.execute(
            f"CREATE TABLE IF NOT EXISTS {self.MAP_TABLE} (memory_id TEXT PRIMARY KEY, fts_rowid INTEGER UNIQUE NOT NULL)"
        )
        connection.execute(
            f"CREATE VIRTUAL TABLE IF NOT EXISTS {self.TABLE} USING fts5(content, tags, content='', contentless_delete=1)"
        )
    ...
    def index(self, connection: sqlite3.Connection, memory_id: str, content: str, tags: str) -> None:
        ...
        existing = connection.execute(
            f"SELECT fts_rowid FROM {self.MAP_TABLE} WHERE memory_id=?", (memory_id,)
        ).fetchone()
        if existing is not None:
            connection.execute(
                f"DELETE FROM {self.TABLE} WHERE rowid=?", (existing[0],)
            )
            connection.execute(f"DELETE FROM {self.MAP_TABLE} WHERE memory_id=?", (memory_id,))
        cursor = connection.execute(
            f"INSERT INTO {self.TABLE}(content, tags) VALUES (?, ?)", (content, tags)
        )
        connection.execute(
            f"INSERT INTO {self.MAP_TABLE}(memory_id, fts_rowid) VALUES (?, ?)",
    ...
    def candidate_ids(self, connection: sqlite3.Connection, query: str, limit: int = 50) -> tuple[str, ...]:
        ...
        rows = connection.execute(
            f"SELECT m.memory_id FROM {self.TABLE} f JOIN {self.MAP_TABLE} m ON m.fts_rowid=f.rowid WHERE f MATCH ? ORDER BY bm25(f) LIMIT ?",
            (query, limit),
        ).fetchall()
