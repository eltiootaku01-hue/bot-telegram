"""OrquestaciÃ³n local de inventario, filtros, ranking y EvidencePack."""
...
class LocalLibrarian:
    def retrieve(
        ...
        scoped = tuple(
            ...
            if entry.record.universe_id == query.universe_id
        )

        allowed = tuple(
            entry
            for entry in scoped
            if is_allowed(entry, query)
        )
            ...
            sorted(
                (
                    self._rank(
                        ...
                        fragments,
                        max(
                            fragment.lexical_score
                            for fragment in fragments
                        ),
                        query,
                        preferred=entry.record.source_id in preferred,
                    )
                    for entry, fragments in matched
                ),
                key=lambda item: (
                    -item.score,
                    item.entry.record.source_id,
                ),
            )
        )

        fragment_candidates: list[tuple[float, int, Fragment]] = []
        ...
        for entry, source_fragments in matched:
            ...
            source_rank = next(
                (
                    item
                    for item in ranked
                    if item.entry.record.source_id
            ...
            for fragment in source_fragments:
                ...
                if first_line.startswith("### "):
                    heading_terms = set(
                        query_terms(first_line[4:].strip())
                    )
                    query_term_set = set(
                        query_terms(query.text)
                    )

                    if (
                        query_term_set
                        and query_term_set.issubset(heading_terms)
                    ):
                        heading_bonus = 0.50

                fragment_score = round(
                    fragment.lexical_score
                    + source_bonus
                    + preferred_bonus
                    + section_bonus
        ...
        fragments = tuple(
            item[2]
            for item in fragment_candidates[: query.max_fragments]
        )

        conflicts = self._conflicts(
            tuple(item.entry for item in ranked)
        )
        ...
        confidence = (
            Confidence.LOW
            if conflicts
            or coverage.status is not CoverageStatus.ESTABLISHED
    ...
    @staticmethod
    def _section_epistemic(section: str) -> tuple[
        EvidenceStatus,
        EpistemicStatus,
        Confidence,
    ]:
        ...
        if normalized in {
            ...
            return (
                EvidenceStatus.FOUND,
                EpistemicStatus.NOT_ESTABLISHED,
                Confidence.MEDIUM,
            )
    ...
    def _rank(
        ...
        *,
        ...
        section_decisions = [
            self._section_epistemic(fragment.section)
            for fragment in fragments
        ]
        ...
        established = [
            ...
            if decision[1] is EpistemicStatus.ESTABLISHED
        ]

        if established:
            evidence = EvidenceStatus.FOUND
            ...
            section_found = [
                decision
                for decision in section_decisions
                if decision[0] is EvidenceStatus.FOUND
            ]

            if section_found:
                evidence = EvidenceStatus.FOUND
                epistemic = EpistemicStatus.NOT_ESTABLISHED
                confidence = Confidence.MEDIUM
            else:
                evidence = base_decision.evidence
                epistemic = base_decision.epistemic
        ...
        evidence_weight = (
            0.20
            if evidence is EvidenceStatus.FOUND
            else 0.03
        )

        epistemic_weight = (
            0.15
            if epistemic is EpistemicStatus.ESTABLISHED
            else 0.02
        )
        ...
        is_identity_query = (
            ...
            and normalized_query[0] not in {'arco', 'saga'}
        )

        is_character_source = (
            entry.record.source_id == 'world__canon-one-punch-man__personajes.'
        )

        if is_identity_query and is_character_source:
            for fragment in fragments:
                first_line = fragment.text.splitlines()[0].strip()
                ...
                if (
                    normalized_query
                    and normalized_query[0] in heading_terms
                    and fragment.lexical_score >= 1.0
                ):
                    identity_bonus = 0.30
                    break
        ...
        reasons = (
            ...
            f"authority={entry.record.authority.value}",
            f"evidence={evidence.value}",
            f"epistemic={epistemic.value}",
            f"preferred_source={preferred}",
            f"version={entry.version[:12]}",
        )
    ...
    def _conflicts(
        ...
        for key, group in sorted(groups.items()):
            if (
                len(group) > 1
                and len({item.version for item in group}) > 1
            ):
                conflicts.append(
                    EvidenceConflict(
                        key,
                        ...
                        "multiple incompatible versions share conflict key",
                    )
                )
    ...
    def _coverage(
        ...
        if conflicts:
            ...
            item.epistemic is EpistemicStatus.ESTABLISHED
            for item in ranked
        ):
