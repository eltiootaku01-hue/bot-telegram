...
class LocalLibrarian:
    def retrieve(
            ...
            sorted(
                (
                    self._rank(
                        entry,
                        fragments,
                        max(fragment.lexical_score for fragment in fragments),
                        query,
        ...
        for entry, source_fragments in matched:
            ...
            source_rank = next(
                (item for item in ranked if item.entry.record.source_id == entry.record.source_id),
                None,
            )
            ...
            for fragment in source_fragments:
                ...
                fragment_score = round(
                    fragment.lexical_score + source_bonus + preferred_bonus + section_bonus + heading_bonus,
                    4,
                )
                fragment_candidates.append((fragment_score, -fragment.start_line, fragment))

        fragment_candidates.sort(
            key=lambda item: (-item[0], item[1], item[2].source_id, item[2].ordinal)
        )
        ...
        confidence = (
            Confidence.LOW
            if conflicts or coverage.status is not CoverageStatus.ESTABLISHED
        ...
        for key, group in sorted(groups.items()):
            if len(group) > 1 and len({item.version for item in group}) > 1:
                conflicts.append(
                    EvidenceConflict(
                        key,
                        tuple(sorted(item.record.source_id for item in group)),
    ...
    def _coverage(
        ...
        if conflicts:
            ...
            status = CoverageStatus.ESTABLECIDO
