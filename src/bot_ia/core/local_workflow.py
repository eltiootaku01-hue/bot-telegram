...
from bot_ia.agents import AgentRegistry, AgentRequest, AgentResult, PolicyRule, RuleCheck, RuleHierarchy, RulePriority, RuleResolution
...
class LocalWorkflow:
    ...
    *,
    ...
    def execute(self, request: ApplicationRequest, brain: BrainResult, decision: RouteDecision) -> LocalExecution:
        ...
        provider_response = self._run_local_provider(
            decision,
            brain,
            agent,
            context,
        ...
        if decision.requires_search:
            gated_text = self._evidence_gate.build_factual_answer(
                evidence
            )
