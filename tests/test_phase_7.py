...
class Phase7Tests(unittest.TestCase):
    ...
    def test_missing_token_is_rejected_and_long_message_is_split(self) -> None:
        with self.assertRaises(TelegramConfigurationError):
            TelegramApiClient("")
        calls = []
        client = TelegramApiClient("test-token", transport=lambda url, payload, timeout: calls.append(payload) or {"ok": True})
        client.send(TelegramOutbound("9", "x" * 4097))
        self.assertEqual(2, len(calls))
        self.assertEqual(4096, len(calls[0]["text"]))
        self.assertEqual(1, len(calls[1]["text"]))
