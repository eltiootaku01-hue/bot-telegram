...
def test_card_art_manifest_matches_v2_matrix_contract() -> None:
    ...
    for item in data["items"]:
        assert item["framing_profile"] in valid_tiers
        assert item["visual_audit_status"] in {"not_inspected", "approved", "failed"}
        assert isinstance(item["adult_eligible"], bool)
        expected = ["normal", "ur-alt-holo"] if item["art_tier"] == "UR" else ["normal"]
        assert item["variant_support"] == expected
