...
class ProviderResponseCacheTests(unittest.TestCase):
    ...
    def test_same_query_context_different_agents_do_not_share_cache(self) -> None:
        workflow, manager = self._workflow()
        workflow._run_local_provider(*self._inputs("ia_chan"))
        workflow._run_local_provider(*self._inputs("editor"))
        self.assertEqual(2, manager.calls)

    def test_same_agent_query_context_reuses_cache(self) -> None:
        workflow, manager = self._workflow()
        inputs = self._inputs("ia_chan")
