...
from bot_ia.providers import OpenAIProvider, ProviderManager, ProviderStatus
...
class FakeOpenAITransport:
    ...
    def __call__(self, url, headers, payload, timeout):
        ...
        return {
            "output_text": "Respuesta generada por OpenAI.",
...
class Phase10ProviderIntegrationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.registry.register(UniverseDefinition("alpha_world", "Alpha", root / "alpha_world"))
        folder = root / "alpha_world"
        folder.mkdir()
        (folder / "fact.md").write_text("Kuro protege Alpha.", encoding="utf-8")
        entries = {
            "alpha_world": SourceInventory(folder).discover(
                "alpha_world",
                {"fact.md": SourceMetadata("alpha_fact", SourceType.CHAPTER, AuthorityLevel.PRIMARY, SourceStatus.VALIDATED, CanonStatus.CANON)},
            )
        }
        self.transport = FakeOpenAITransport()
        openai = OpenAIProvider(key_loader=lambda _: "test-key", transport=self.transport)
        self.memory = MemoryStore(root, self.registry)
        self.workflow = LocalWorkflow(
            entries,
            memory_store=self.memory,
            provider_manager=ProviderManager((openai,)),
            provider_id="openai",
    ...
    def handle(self, message: str):
        return self.app.handle(ApplicationRequest("u1", "c1", message))

    def test_llm_route_reaches_configured_openai(self) -> None:
        ...
        self.assertEqual("openai", provider_response.provider)
        self.assertEqual(ProviderStatus.SUCCESS, provider_response.status)
        self.assertEqual("Respuesta generada por OpenAI.", response.text)

    def test_openai_receives_prepared_agent_output(self) -> None:
        response = self.handle("Escribe una escena.")
        self.assertEqual(1, len(self.transport.calls))
        payload = self.transport.calls[0]["payload"]
        self.assertEqual("openai-test", payload["model"])
        text = payload["input"]
        ...
        self.assertEqual("openai-test", response.execution.provider_response.model)
    ...
    def test_local_route_does_not_call_openai(self) -> None:
    ...
    def test_provider_configuration_is_not_stored_in_request_content(self) -> None:
        ...
        self.assertIsNotNone(provider_response)
        self.assertEqual("openai", provider_response.provider)
        self.assertEqual("openai-test", provider_response.model)
